function processar() {
    ...
    if (!codigo) { statusMsg.innerText = "Erro: informe o código de barras."; statusMsg.className = "status erro"; return; }
    try {
        let prodCalculado = "—"; let precoCalculado = "—";
        if (iniProd > 0 && tamProd > 0 && codigo.length >= (iniProd - 1 + tamProd)) { prodCalculado = codigo.substring(iniProd - 1, (iniProd - 1) + tamProd); }
    ...
    } catch (e) { statusMsg.innerText = "Erro: " + e.message; statusMsg.className = "status erro"; }
}
function limparCampos() {
    ...
    document.getElementById('resCodInf').innerText = "—"; document.getElementById('statusMsg').innerText = ""; document.getElementById('statusMsg').className = "status";
}